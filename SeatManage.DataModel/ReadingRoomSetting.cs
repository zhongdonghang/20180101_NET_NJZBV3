    ...
    public class ReadingRoomSetting
    {
        ...
        public List<DateTime> GetFreeChoiceTimeList(DateTime date, int interval)
        {
            ...
            while (true)
            {
                ...
                if (ReadingRoomOpenState(time) == SystemCode.EnumType.ReadingRoomStatus.Close)
                {
                    continue;
                }
                timeList.Add(time);
        ...
        {
            return this.SeatBespeak.SpecifiedTimeList.Where(time => DateTime.Parse(date.ToLongDateString() + " " + time.ToLongTimeString()) > DateTime.Now).Where(time => ReadingRoomOpenState(time) != SystemCode.EnumType.ReadingRoomStatus.Close).ToList();
        }
        ...
        public bool IsCanBespeakSeat(DateTime time)
        {
            if (this.SeatBespeak.Used)
            {
                ...
                //{
                if (time.Day > DateTime.Now.AddDays(this.SeatBespeak.BespeakBeforeDays).Day)
                {
                    return false;
                }
