    ...
    public class AppLoadingWindow_ViewModel : INotifyPropertyChanged
    {
        ...
        private bool CheckNetWork()
        {
            try
            {
                ...
                if (!SeatManage.SeatManageComm.CheckInternet.CheckLocal())
                {
                ...
                {
                    endpointAddress = SeatManage.SeatManageComm.AESAlgorithm.AESDecrypt(ConfigurationManager.ConnectionStrings["EndpointAddress"].ConnectionString);
                    ...
                    if (!SeatManage.SeatManageComm.CheckInternet.CheckPort(straddress.Split(':')[0], straddress.Split(':')[1]))
                    {
