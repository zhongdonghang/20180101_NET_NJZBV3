...
namespace SeatClientV3
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void timeLoop_TimeTo(object sender, EventArgs e)
        {
            count--;//倒计时,count开始是15,每次减1
            this.btnGetNo_Copy.Dispatcher.Invoke(
                //更换按钮上的文字开始
                new Action(   
                  delegate
                  {
                      btnGetNo_Copy.Content = "刷卡中(" + count + ")";
                  }
             )
            );//更换按钮上的文字结束
            if (count == 0) //如果15秒结束
            {
                timeLoop.TimeTo -= new EventHandler(timeLoop_TimeTo);//注销定时器触发的事件
                timeLoop.TimeStop();//停止定时器
                IsWork = false;//停止工作的标识
                count = 15;//重新初始化倒计时的数值
                StopRead();//停止读卡
                //更换按钮上的文字开始
                this.btnGetNo_Copy.Dispatcher.Invoke(
                new Action(
