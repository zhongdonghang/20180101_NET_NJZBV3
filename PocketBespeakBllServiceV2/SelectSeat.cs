...
namespace SeatManage.PocketBespeakBllServiceV2
{
    public partial class PocketBespeakBllService : IPocketBespeakBllServiceV2.IPocketBespeakBllService
    {
        ...
        public bool IsPushAccount(string cardNo)
        {
            ...
            strSql.AppendFormat("SELECT [ID],[VisitID],[VisitTime],[CardNo],[CardID],[VisierName],[Library],[EnterOrExit],[ChannelNo],[Flag],[HandleFlag] FROM [AccessSyncLog] where  datediff(mi,VisitTime,getdate())<{0} and CardNo='{1}'", effectiveTime, cardNo);
            try
            {
