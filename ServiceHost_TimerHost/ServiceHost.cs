        ...
        protected override void OnStart(string[] args)
        {
            try
            {
                ...
                foreach (IService.IService service in services)
                {
                    ...
                    {
                        service.Start();
                        SeatManage.SeatManageComm.WriteLog.Write(string.Format("“{0}”已启动", service.ToString()));
                    }
                    catch (Exception ex)
                    {
                        SeatManage.SeatManageComm.WriteLog.Write(string.Format("“{0}”启动遇到异常：{1}", service.ToString(), ex.Message));
