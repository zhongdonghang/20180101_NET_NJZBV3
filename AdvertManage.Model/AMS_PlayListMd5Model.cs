...
namespace AdvertManage.Model
{
    ...
    /// </summary>
    [Serializable]
    public class AMS_PlayListMd5Model : AMS_PlayListModel
    {

        public AMS_PlayListMd5Model()
        {
            PlayListNo = "";
            PlayVideoItems = new List<AMS_VideoMd5Item>();
        ...
        /// 视频文件播放列表
        /// </summary>
        public List<AMS_VideoMd5Item> PlayVideoItems
        {
            get;
            set;
        }
        ...
        /// 视频文件
        /// </summary>
        public List<AMS_VideoMd5Item> VideoFiles
        {
            get;
            set;
        }
        ...
        /// <returns></returns>
        public static AMS_PlayListMd5Model Parse(string xmlPlayList)
        {
            AMS_PlayListMd5Model plm = new AMS_PlayListMd5Model();
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                //载入字符串类型的XML
        ...
        /// <param name="xmlPlayList">xml结构的播放列表</param>
        /// <returns></returns>
        private static AMS_PlayListMd5Model Parse(XmlDocument xmlPlayList)
        {
            AMS_PlayListMd5Model plm = new AMS_PlayListMd5Model();
            ...
            try
            {
                ...
                foreach (XmlNode n in nodes)
                {
                    ...
                    if (i >= videoFiles.Count)
                    {
                        ...
                        if ((n.Attributes["md5value"]) == null)
                        {
                            videoFiles.Add(new AMS_VideoMd5Item() { Name = n.Attributes["name"].Value, RelativeUrl = n.Attributes["source"].Value, md5value = null });
                        }
                        else
                        {
                            videoFiles.Add(new AMS_VideoMd5Item() { Name = n.Attributes["name"].Value, RelativeUrl = n.Attributes["source"].Value, md5value = n.Attributes["md5value"].Value });
                        }
