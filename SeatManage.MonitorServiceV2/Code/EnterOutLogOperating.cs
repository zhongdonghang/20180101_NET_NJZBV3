    ...
    public partial class SeatDataOperation
    {
        ...
        public void EnterOutLogOperate()
        {
            try
            {
                ...
                List<EnterOutLogInfo> eolList = seatManageService.GetEnterOutLogsByStatus(null, null, null, enterOutLogTypeList, LogStatus.Valid, nowDateTime.ToShortDateString(), null);
                            ...
                            case EnterOutLogType.ShortLeave:
                                {
                                    ...
                                    {
                                        //判断座位等待处理
                                        List<WaitSeatLogInfo> wslilist = seatManageService.GetWaitLogList(null, eol.EnterOutLogID, null, null, new List<EnterOutLogType> { EnterOutLogType.Waiting });
                                        if (wslilist.Count > 0)
                                        {
                                    ...
                                    else
                                    {
                                        if (eol.Flag == Operation.Admin)
                                        {
                                            ...
                                            else if (!rri.Setting.AdminShortLeave.IsUsed && (eol.EnterOutTime.AddMinutes(rri.Setting.GetSeatHoldTime(eol.EnterOutTime)) < nowDateTime))
                                            {
                                                ShortLeaveOverTimeOperator(eol, rri.Setting, nowDateTime);
                                            }
                                        }
                                        else if (eol.EnterOutTime.AddMinutes(rri.Setting.GetSeatHoldTime(eol.EnterOutTime)) < nowDateTime)
                                        {
                                            ShortLeaveOverTimeOperator(eol, rri.Setting, nowDateTime);
                                        }
                                    }
        ...
        private void SeatOverTimeOperator(ReadingRoomSetting roomSetting, EnterOutLogInfo eol, DateTime nowDateTime)
        {
            try
            {
                int logdi = 0;
        ...
        /// </summary>
        /// <param name="eol">进出记录</param>
        private EnterOutLogInfo GetLastNoSeatTimeLog(EnterOutLogInfo eol)
        {
            try
            {
                ...
                List<EnterOutLogInfo> lasteol = seatManageService.GetEnterOutLogsByNo(eol.EnterOutLogNo, eoltypeList, 1);
                if (lasteol.Count > 0)
                {
                    return lasteol[0];
                }
