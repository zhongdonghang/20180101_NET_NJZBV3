    ...
    public partial class SeatDataOperation
    {
        public void BeapeakLogOperating()
        {
            try
            {
                ...
                //预约超时处理
                List<BespeakLogInfo> blilist = seatManageService.GetNotCheckedBespeakLogInfo(null, nowDateTime);
                if (blilist.Count <= 0)
                {
                    return;
                }
        ...
        private void BookingOverTime(ReadingRoomSetting roomSetting, DateTime NowDateTime, BespeakLogInfo bli)
        {
            try
            {
