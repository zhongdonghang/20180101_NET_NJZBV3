...
namespace SeatService.MonitorService.Code
{
    public partial class SeatDataOperation
    {
        public void ClearSeat()
        {
            ...
            {
                //获取昨天的进出记录
                List<EnterOutLogInfo> eolList = seatManageService.GetEnterOutLogsByStatus(null, null, null, enterOutLogTypeList, LogStatus.Valid, "1900-1-1", DateTime.Now.ToShortDateString());
                foreach (EnterOutLogInfo eol in eolList)
                {
                    ...
                    {
                        //获取昨天的等待记录
                        List<WaitSeatLogInfo> wsllist = seatManageService.GetWaitLogList(null, eol.EnterOutLogID, null, null, new List<EnterOutLogType> { EnterOutLogType.Waiting });
                        if (wsllist.Count > 0)
                        {
                            wsllist[0].WaitingState = EnterOutLogType.WaitingCancel;
                            wsllist[0].StatsChangeTime = nowDateTime;
                            seatManageService.UpdateWaitLog(wsllist[0]);
                        }
                    }
                    ...
                    int logdi = 0;
                    seatManageService.AddEnterOutLogInfo(eol, ref logdi);
                    WriteLog.Write(string.Format("监控服务：读者{0}，{1}", eol.CardNo, eol.Remark));
                }
                //预约记录处理
                TimeSpan span = nowDateTime - DateTime.Parse("2010-1-1");
                List<BespeakLogInfo> blilist = seatManageService.GetBespeakLogs(null, null, nowDateTime.AddDays(-1), span.Days, new List<BookingStatus> { BookingStatus.Waiting });
                if (blilist.Count <= 0)
                {
                    return;
                }
                foreach (BespeakLogInfo bli in blilist.Where(bli => bli.BsepeakTime.Date < nowDateTime.Date))
                {
                    ...
                    seatManageService.UpdateBespeakLogInfo(bli);
        ...
        public void OpenCloseReadingRoom()
        {
            ...
            {
                DateTime nowDateTime = DateTime.Now;
                List<ReadingRoomOpenCloseLogInfo> rrocList = seatManageService.GetReadingRoomOCLog(null, LogStatus.Valid, null, null);
                //遍历所有阅览室
                foreach (ReadingRoomInfo rri in roomList.Values.Where(rri => rri.Setting != null))
                {
                    ...
                    if (rri.Setting.RoomOpenSet.UninterruptibleModel)
                    {
                        if (rroc.OpenCloseState == ReadingRoomStatus.Close)
                        {
                            ...
                            rroc.OperateTime = nowDateTime;
                            seatManageService.AddReadingRoomOClog(rroc, ref new_id);
        ...
        private void OpenReadingRoom(ReadingRoomInfo room)
        {
            ...
            {
                //获取昨天的进出记录
                List<EnterOutLogInfo> eolList = seatManageService.GetEnterOutLogsByStatus(null, room.No, null, enterOutLogTypeList, LogStatus.Valid, "1900-1-1", DateTime.Now.ToShortDateString());
                foreach (EnterOutLogInfo eol in eolList)
                {
                    if ((eol.EnterOutState == EnterOutLogType.ShortLeave) && (eol.Flag == Operation.Admin || eol.Flag == Operation.OtherReader))
                    {
                ...
                //预约记录处理
                TimeSpan span = nowDateTime - DateTime.Parse("2010-1-1");
                List<BespeakLogInfo> blilist = seatManageService.GetBespeakLogs(null, room.No, nowDateTime.AddDays(-1), span.Days, new List<BookingStatus> { BookingStatus.Waiting });
                if (blilist.Count <= 0)
                {
                    return;
                }
        ...
        private void CloseReadingRoom(ReadingRoomInfo room)
        {
            ...
            {
                //获取今天的进出记录
                List<EnterOutLogInfo> eolList = seatManageService.GetEnterOutLogsByStatus(null, room.No, null, enterOutLogTypeList, LogStatus.Valid, nowDateTime.ToShortDateString(), null);
                foreach (EnterOutLogInfo eol in eolList)
                {
                    if ((eol.EnterOutState == EnterOutLogType.ShortLeave) && (eol.Flag == Operation.Admin || eol.Flag == Operation.OtherReader))
                    {
                    ...
                    //WcfServiceForSeatManage.T_SM_ReaderNotice.SendPushMsg(msg);
