    ...
    public class EnterOutLogUpload
    {
        ...
        public static bool Upload()
        {
            ...
            foreach (SeatManage.ClassModel.EnterOutLogInfo model in enterOutLogList)
            {
                ...
                amsModel.EnterOutState = (Model.Enum.EnterOutLogType)(int)model.EnterOutState;
                amsModel.EnterOutTime = model.EnterOutTime;
                amsModel.EnterOutType = (Model.Enum.LogStatus)(int)model.EnterOutType;
                amsModel.Operator = (Model.Enum.Operation)(int)model.Flag;
                amsModel.ReadingRoomNum = model.ReadingRoomNo;
                amsModel.Remark = model.Remark;
                amsModel.SchoolNum = ServiceSet.SchoolNums;
            ...
            if (ams_enterOutLogList.Count > 0 && BLL.AMS_EnterOutLogBLL.AddEnterOutLogList(ams_enterOutLogList) == Model.Enum.HandleResult.Successed)
            {
                writeMaxId(newMaxId);
                return true;
            }
