...
namespace SeatManage.MobileAppService
{
    ...
    public class MobileAppService : IService.IService
    {
        ...
        {
            return "座位管理系统移动客户端访问服务";
        }

        SocketClient client;
        ServerIp ip = new ServerIp(ConfigurationManager.ConnectionStrings["ip"].ConnectionString);
        ...
        private void client_OnMsgReceived(byte[] info)
        {
            ...
            if (requestMsg != null)
            {
                ...
                try
                {
                    result = OperationFactory.Execute(requestMsg.Parameters, requestMsg.RequestMethod);
