            ...
            try
            {
                ...
                foreach (SeatManage.ClassModel.Seat seat in _SeatLayout.Seats.Values)
                {
                    ...
                    else if (seat.CanBeBespeak)
                    {
                        ...
                        {
                            if (seat.CanBespeakSpan.Count > 0)
                            {
                                tipContent = "可预约";
                                urlParameters = string.Format("seatNo={0}&seatShortNo={1}&date={2}&roomNo={3}&timeSpan={4}", seat.SeatNo, seat.ShortSeatNo, date.ToBinary(), roomNum, seat.CanBespeakStr);
                            }
                            else
                            {
