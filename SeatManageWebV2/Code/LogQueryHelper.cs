    ...
    public class LogQueryHelper
    {
        ...
        public static DataTable GetMonitorGraphReadingRoomList(string LoginID)
        {
            ...
            foreach (ReadingRoomInfo model in potency.RightRoomList)
            {
                ...
                //    }
                //}
                dr["seatCountAll"] = roomSeatUsingState[model.No].SeatAmountAll;
        ...
        public static DataTable BespeakRoomList(DateTime date, string libNo)
        {
            ...
            {
                if(!seatBespeakState.ContainsKey(room.No))
                {
                    continue;
                }
                DataRow dr = dt.NewRow();
        ...
        public static DataTable LibrarySeatInfo(string libNum)
        {
            ...
            {
                if (!roomSeatUsingState.ContainsKey(rooms[i].No))
                {
                    continue;
                }
                DataRow dr = dt.NewRow();
                ...
                dr["LeisureSeat"] = roomSeatUsingState[rooms[i].No].SeatAmountAll - roomSeatUsingState[rooms[i].No].SeatAmountUsed - roomSeatUsingState[rooms[i].No].SeatBookingCount + roomSeatUsingState[rooms[i].No].SeatTemUseCount;
                dt.Rows.Add(dr);
        ...
        public static DataTable NowDayBespeakRoomInfo(string libNum)
        {
            ...
            for (int i = 0; i < rooms.Count; i++)
            {
                if (!rooms[i].Setting.SeatBespeak.Used || !rooms[i].Setting.SeatBespeak.NowDayBespeak)
                {
                    continue;
                }
                DataRow dr = dt.NewRow();
