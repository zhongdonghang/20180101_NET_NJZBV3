...
namespace SeatManageWebV2.Code
{
    public class ReadingRoomStatistics
    {
        ...
        /// <returns></returns>
        public HtmlTable GetReadingRoomUsageInfo(string roomNo,string readerType, DateTime starttime, DateTime endtime)
        {
            starttime = DateTime.Parse(starttime.Date.ToShortDateString() + " 0:00:00");
            endtime = DateTime.Parse(endtime.Date.ToShortDateString() + " 23:59:59");
            ...
            List<SeatManage.ClassModel.EnterOutLogStatistics> typelist;
            ...
            {
                typelist = staList.FindAll(u => u.TypeName == readerType);
            }
            foreach (SeatManage.ClassModel.EnterOutLogStatistics eols in typelist)
            {
            ...
            try
            {
                switch (logtype)
                {
                    case TopLogType.EnterOutLog:
                        List<SeatManage.ClassModel.EnterOutLogStatistics> staList = SeatManage.Bll.T_SM_EnterOutLogStatistics.GetEnterOutLogStatisticsByDate(null, starttime.ToString(), endtime.ToString());
                        ...
                        switch (type)
                        {
                            case TopReaderType.OnlyReader:
                                foreach (SeatManage.ClassModel.EnterOutLogStatistics sta in staList)
                                {
                                    if (Progress != null)
                                    {
                                        Progress("(1/4)正在处理数据请耐心等待……" + (float.Parse(staList.IndexOf(sta).ToString()) / float.Parse(staList.Count.ToString()) * 100).ToString("0.00") + "%");
                                    }
                        ...
                        for (int i = 0; i < topDataInfoList.Count; i++)
                        {
                            if (Progress != null)
                            {
                                Progress("(2/4)正在对数据进行排序请耐心等待……" + (float.Parse(i.ToString()) / float.Parse(topDataInfoList.Count.ToString()) * 100).ToString("0.00") + "%");
                            }
                    ...
                    case TopLogType.SeatTime:
                        List<SeatManage.ClassModel.EnterOutLogStatistics> staListst = SeatManage.Bll.T_SM_EnterOutLogStatistics.GetEnterOutLogStatisticsByDate(null, starttime.ToString(), endtime.ToString());
                        ...
                        switch (type)
                        {
                            case TopReaderType.OnlyReader:
                                foreach (SeatManage.ClassModel.EnterOutLogStatistics sta in staListst)
                                {
                                    if (Progress != null)
                                    {
                                        Progress("(1/4)正在处理数据请耐心等待……" + (float.Parse(staListst.IndexOf(sta).ToString()) / float.Parse(staListst.Count.ToString()) * 100).ToString("0.00") + "%");
                                    }
                        ...
                        for (int i = 0; i < topDataInfoListst.Count; i++)
                        {
                            if (Progress != null)
                            {
                                Progress("(2/4)正在对数据进行排序请耐心等待……" + (float.Parse(i.ToString()) / float.Parse(topDataInfoListst.Count.ToString()) * 100).ToString("0.00") + "%");
                            }
                    ...
                    case TopLogType.ViolateDiscipline:
                        List<SeatManage.ClassModel.ViolationRecordsLogInfo> vrloglist = SeatManage.Bll.T_SM_ViolateDiscipline.GetViolationRecords(null, null, starttime.ToString(), endtime.ToString(), SeatManage.EnumType.LogStatus.None, SeatManage.EnumType.LogStatus.None);
                        ...
                        switch (type)
                        {
                            case TopReaderType.OnlyReader:
                                foreach (SeatManage.ClassModel.ViolationRecordsLogInfo sta in vrloglist)
                                {
                                    if (Progress != null)
                                    {
                                        Progress("(1/4)正在处理数据请耐心等待……" + (float.Parse(vrloglist.IndexOf(sta).ToString()) / float.Parse(vrloglist.Count.ToString()) * 100).ToString("0.00") + "%");
                                    }
                                    ...
                                    topDataListvr[sta.CardNo].ViolateDisciplineConnt[sta.EnterFlag]++;
                                };
                                break;
                            case TopReaderType.ReaderDept:
                                foreach (SeatManage.ClassModel.ViolationRecordsLogInfo sta in vrloglist)
                                {
                                    ...
                                    topDataListvr[sta.DeptName].ViolateDisciplineConnt[sta.EnterFlag]++;
                                }
                                break;
                            case TopReaderType.ReaderType:
                                foreach (SeatManage.ClassModel.ViolationRecordsLogInfo sta in vrloglist)
                                {
                                    ...
                                    topDataListvr[sta.TypeName].ViolateDisciplineConnt[sta.EnterFlag]++;
                                }
                                break;
                        }
                        ...
                        for (int i = 0; i < topDataInfoListvr.Count; i++)
                        {
                            if (Progress != null)
                            {
                                Progress("(2/4)正在对数据进行排序请耐心等待……" + (float.Parse(i.ToString()) / float.Parse(topDataInfoListvr.Count.ToString()) * 100).ToString("0.00") + "%");
                            }
                        ...
                        for (int i = 0; i < topDataInfoListvr.Count; i++)
                        {
                            ...
                            dr["Remark"] = "预约超时" + topDataInfoListvr[i].ViolateDisciplineConnt[SeatManage.EnumType.ViolationRecordsType.BookingTimeOut]
                                + "次，在座超时" + topDataInfoListvr[i].ViolateDisciplineConnt[SeatManage.EnumType.ViolationRecordsType.SeatOutTime]
                                + "次，暂离超时" + (topDataInfoListvr[i].ViolateDisciplineConnt[SeatManage.EnumType.ViolationRecordsType.ShortLeaveOutTime] + topDataInfoListvr[i].ViolateDisciplineConnt[SeatManage.EnumType.ViolationRecordsType.ShortLeaveByAdminOutTime] + topDataInfoListvr[i].ViolateDisciplineConnt[SeatManage.EnumType.ViolationRecordsType.ShortLeaveByReaderOutTime])
                                + "次,被管理员释放座位" + topDataInfoListvr[i].ViolateDisciplineConnt[SeatManage.EnumType.ViolationRecordsType.LeaveByAdmin]
                                + "次，离开未刷卡" + topDataInfoListvr[i].ViolateDisciplineConnt[SeatManage.EnumType.ViolationRecordsType.LeaveNotReadCard]
                                + "次";
                            dt.Rows.Add(dr);
                        }
                        break;
                    case TopLogType.Blastlist:
                        List<SeatManage.ClassModel.BlackListInfo> blacklistlist = SeatManage.Bll.T_SM_Blacklist.GetAllBlackListInfo(null, SeatManage.EnumType.LogStatus.None, starttime.ToString(), endtime.ToString());
                        ...
                        switch (type)
                        {
                            case TopReaderType.OnlyReader:
                                foreach (SeatManage.ClassModel.BlackListInfo sta in blacklistlist)
                                {
                                    if (Progress != null)
                                    {
                                        Progress("(1/4)正在处理数据请耐心等待……" + (float.Parse(blacklistlist.IndexOf(sta).ToString()) / float.Parse(blacklistlist.Count.ToString()) * 100).ToString("0.00") + "%");
                                    }
                        ...
                        for (int i = 0; i < topDataInfoListbl.Count; i++)
                        {
                            if (Progress != null)
                            {
                                Progress("(2/4)正在对数据进行排序请耐心等待……" + (float.Parse(i.ToString()) / float.Parse(topDataInfoListbl.Count.ToString()) * 100).ToString("0.00") + "%");
                            }
        ...
        public class TopDataInfo
        {
            public TopDataInfo()
            {
                ...
                _ViolateDisciplineConnt.Add(SeatManage.EnumType.ViolationRecordsType.BookingTimeOut, 0);
                _ViolateDisciplineConnt.Add(SeatManage.EnumType.ViolationRecordsType.LeaveByAdmin, 0);
                _ViolateDisciplineConnt.Add(SeatManage.EnumType.ViolationRecordsType.LeaveNotReadCard, 0);
                _ViolateDisciplineConnt.Add(SeatManage.EnumType.ViolationRecordsType.SeatOutTime, 0);
                _ViolateDisciplineConnt.Add(SeatManage.EnumType.ViolationRecordsType.ShortLeaveByAdminOutTime, 0);
                _ViolateDisciplineConnt.Add(SeatManage.EnumType.ViolationRecordsType.ShortLeaveByReaderOutTime, 0);
                _ViolateDisciplineConnt.Add(SeatManage.EnumType.ViolationRecordsType.ShortLeaveOutTime, 0);
            }
            ...
            public int AllViolateDisciplineCount
            {
                ...
                {
                    int count = 0;
                    foreach (KeyValuePair<SeatManage.EnumType.ViolationRecordsType, int> item in _ViolateDisciplineConnt)
                    {
                        count += item.Value;
                    }
                    return count;
            ...
            private Dictionary<SeatManage.EnumType.ViolationRecordsType, int> _ViolateDisciplineConnt = new Dictionary<SeatManage.EnumType.ViolationRecordsType, int>();
            /// <summary>
            /// 违规类型次数
            /// </summary>
            public Dictionary<SeatManage.EnumType.ViolationRecordsType, int> ViolateDisciplineConnt
            {
