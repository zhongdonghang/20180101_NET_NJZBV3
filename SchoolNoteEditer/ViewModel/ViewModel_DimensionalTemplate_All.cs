...
{
    public class ViewModel_DimensionalTemplate_All : INotifyPropertyChanged
    {
        ...
        private string _Url = "http://wechar.juneberry.cn/User/GetSeatNowStatus";
        /// <summary>
        /// url
        /// </summary>
        public string Url
        ...
        public bool Save()
        {
            ...
            try
            {
                ...
                {
                    string savePath = _SavePath + roomInfo.No + "_" + roomInfo.Name + "\\";
                    ...
                    {
                        //默认
                        foreach (KeyValuePair<string, SeatManage.ClassModel.Seat> seat in roomInfo.SeatList.Seats)
                        {
                            Bitmap dCode = QRCode.GetDimensionalCode(codeURL + AESAlgorithm.AESEncrypt(string.Format(AESCode, seat.Value.SeatNo, roomInfo.No), "SeatManage_WeiCharCode"), 6, 12, "M");
                    ...
                    for (int i = 0; i < seatList.Count; i++)
                    {
                        ...
                        //座位编号二维码
                        Image seatQR = QRCode.GetDimensionalCode(codeURL + AESAlgorithm.AESEncrypt(string.Format(AESCode, seatList[seatOrder].SeatNo, roomInfo.No), "SeatManage_WeiCharCode"), 6, 12, "H");
                        ...
                        if (seatNo == seatCodeCount - 1 || i + 1 == seatList.Count)
                        {
                            ...
                            {
                                gi.DrawImage(DrawText(roomNameList[j], roomInfo.Name), (int) roomNameList[j].PosintionX, (int) roomNameList[j].PosintionY, (int) roomNameList[j].Width, (int) roomNameList[j].Height);
                            }
                            ...
                            image.Save(savePath + roomInfo.Name + imageName + ".jpg", ImageFormat.Jpeg);
                            image.Dispose();
                        }
                    }
                }
