...
namespace SeatService.DataTransfer.Code
{
    ...
    public class EnterOutLogUpload
    {
        ...
        public static bool Upload()
        {
            ...
            List<AMS_EnterOutLog> ams_enterOutLogList = new List<AMS_EnterOutLog>();
            int newMaxId = 0;
            foreach (SeatManage.ClassModel.EnterOutLogInfo model in enterOutLogList)
            {
                AMS.Model.AMS_EnterOutLog amsModel = new AMS.Model.AMS_EnterOutLog();
            ...
            if (ams_enterOutLogList.Count > 0 && AMS.ServiceProxy.IEnterOutLogService.AddEnterOutLogList(ams_enterOutLogList) == AMS.Model.Enum.HandleResult.Successed)
            {
                writeMaxId(newMaxId);
                return true;
            }
