...
namespace SeatService.DataTransfer.Code
{
    public class DataTransfer
    {
        SeatManage.IWCFService.ISeatManageService seatService = new WcfServiceForSeatManage.SeatManageDateService();
        ...
        public void GetDevice()
        {
            try
            {
                if (ServiceSet.IsOnline)
                {
                    ...
                    //学校的设备列表
                    List<TerminalInfoV2> oldModelList = seatService.GetAllTeminalInfo();
                    ...
                    foreach (KeyValuePair<string, AMS_Device> item in ModelDic)
                    {
                        if (oldModelDic.Keys.Contains(item.Key))
                        {
                            ...
                            {
                                TerminalInfoV2 terminal = seatService.GetTeminalInfo(item.Value.Number);
                                //terminal.Describe = item.Value.Describe;
                                terminal.EmpowerLoseEfficacyTime = DateTime.Now.AddDays(7);
                                seatService.UpdateTeminalInfo(terminal);
                            }
                            else
                            {
                                seatService.DeleteTerminal(item.Key);
                            }
                        }
                        else
                        {
                            if (!item.Value.IsDel.Value)
                            {
                                ...
                                seatService.AddTeminalInfo(terminal);
                            }
                        }
                ...
                {
                    SystemAuthorization saModel = seatService.GetSystemAuthorization();
                    if (saModel == null || saModel.SchoolNum != seatService.GetSchoolNum())
                    {
                    ...
                    {
                        item.Value.EmpowerLoseEfficacyTime = DateTime.Now.AddDays(7);
                        seatService.UpdateTeminalInfo(item.Value);
                    }
