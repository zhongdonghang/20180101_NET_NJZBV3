...
namespace SeatService.DataTransfer.Code
{
    public class SlipCustomer
    {
        ...
        public static bool GetSlipCustomer(int num)
        {
            ...
            try
            {
                AMS.Model.AMS_SlipCustomer advertModel = AMS.ServiceProxy.ISlipCustomerService.GetSlipCustomerByID(num);
        ...
        public static bool UploadSlipPrintInfo()
        {
            try
            {
                ...
                List<View_SlipPrintInfo> advertPrintInfo = new List<View_SlipPrintInfo>();
                //TODO:改为直接传递List，
                foreach (SeatManage.ClassModel.AMS_SlipCustomer model in slipCustomerList)
                {
                    AMS.Model.View_SlipPrintInfo slipPrintModel = new AMS.Model.View_SlipPrintInfo();
                ...
                if (AMS.ServiceProxy.ISlipPrintInfoService.AddSlipPrintInfo(advertPrintInfo) == AMS.Model.Enum.HandleResult.Successed)
                {
                    foreach (SeatManage.ClassModel.AMS_SlipCustomer model in slipCustomerList)
                    {
                        model.PrintAmount = 0;
