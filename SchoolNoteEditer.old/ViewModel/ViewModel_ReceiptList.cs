...
{
    public class ViewModel_ReceiptList : INotifyPropertyChanged
    {
        private ObservableCollection<SeatManage.ClassModel.PrintReceiptInfo> _ReceiptList = new ObservableCollection<SeatManage.ClassModel.PrintReceiptInfo>();

        public ObservableCollection<SeatManage.ClassModel.PrintReceiptInfo> ReceiptList
        {
        ...
        public void Delete(int index)
        {
            ...
            {
                string error = "";
                error = SeatManage.Bll.AdvertisementOperation.DeleteAdModel(new SeatManage.ClassModel.AMS_Advertisement() { ID = ReceiptList[index].ID });
                if (error == "")
                {
                    ErrorMessage = "删除成功！";
